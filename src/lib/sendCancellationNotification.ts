...
/**
 * Send cancellation notification emails to both client and owner
 */
export async function sendCancellationNotification(
  ...
  console.log('📧 Attempting to send cancellation notifications...');
  ...
  try {
    ...
    const functionUrl = `${supabaseUrl}/functions/v1/send-cancellation-notification`;
    ...
    console.log('✅ Cancellation notifications sent successfully!', data);
    return { success: true };
  } catch (error) {
    console.error('❌ Error calling cancellation notification function:', error);
    return { 
      success: false, 
